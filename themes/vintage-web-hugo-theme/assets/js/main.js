...
function initRetroEffects() {
    ...
    if (statusBar && Math.random() > 0.7) {
        const message =
            retroMessages[Math.floor(Math.random() * retroMessages.length)];
...
function initCursorTrail() {
    ...
    document.addEventListener("mousemove", function (e) {
        ...
        trail.forEach((point, index) => {
            if (index % 3 === 0) {
                // Only every 3rd point to reduce clutter
...
function initKeyboardNavigation() {
    document.addEventListener("keydown", function (e) {
        ...
        if (e.key === "Escape") {
            document
                .querySelectorAll(".window .window-content")
                .forEach((content) => {
                    if (content.style.display !== "none") {
                        content.style.display = "none";
                    }
        ...
        if (e.key === "F11") {
            e.preventDefault();
            alert(
                "F11 detected! If this were a real 90s browser, you'd now be in fullscreen mode! 📺"
            );
        }
    });
}
...
function initSearch() {
    ...
    if (searchInput) {
        searchInput.addEventListener("input", function () {
            ...
            posts.forEach((post) => {
                const title = post
                    .querySelector(".post-list-title a")
